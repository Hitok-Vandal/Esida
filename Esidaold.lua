...
local jsn_upd = "https://raw.githubusercontent.com/Hitok-Vandal/Esida/main/version.json"
local lect_path = getWorkingDirectory() .. '\\Esida\\Lections.json'
local calc_font = renderCreateFont('Montserrat', 12, 1)
local cfg = inicfg.load({
	...
	govstr = {
		'[LSMC] Уважаемые граждане штата Гилберт, в данный момент ',
		'[LSMC] Проходит Собеседование в мед.центре г.Лос-Сантос. ',
		'[LSMC] При себе иметь: паспорт, лицензии и мед.карту!'
	},
	govdep = {
		'[LSMC] - [ALL] Занимаю гос.волну, просьба не перебивать',
		'[LSMC] - [ALL] Освободил гос.волну вещания.'
	},
	blacklist = {},
	Binds_Name = {},
	Binds_Action = {},
...
local colorBenson        = imgui.ImFloat4(imgui.ImColor(cfg.main.colorBenson):GetFloat4())
...
function main()
	...
	if not doesFileExist('moonloader/config/Esida_Config.ini') then
		if inicfg.save(cfg, 'Esida_Config.ini') then log('Создан файл Esida_Config.ini') end
	end

	if checkData() then
		log('Проверка файлов прошла успешно!')
	end
	...
	log('Готов!')
	bratishka = sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))):find('Benson')
	addNotify("\nСкрипт загружен!\nГлавное меню: {006AC2}/lid", 5)

	if cfg.main.infoupdate then
		infoupdate.v = true
		cfg.main.infoupdate = false
		inicfg.save(cfg, 'Esida_Config.ini')
	end
	...
	if sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))) == 'Alex_Benson' then
		addBankMessage('Вы вошли как разработчик!')
		devmode = true
	end
	
	sampRegisterChatCommand('setpost', function(text)
		if #text > 0 then
			addBankMessage(string.format('Вы изменили название своей должности: {M}%s{W} -> {M}%s{W} ', cfg.nameRank[10], text))
			cfg.nameRank[10] = text
		else
		addBankMessage('Используй /setpost [Должность]')
		end
	end)
	
	sampRegisterChatCommand('setchatid', function(text)
		if #text > 0 then
			addBankMessage(string.format('Вы изменили ID чата беседы: {M}%s{W} -> {M}%s{W} ', cfg.nameRank[8], text))
			cfg.nameRank[8] = text
		else
		addBankMessage('Используй /setchatid [ID Беседы] (Только цифры)')
		end
	end)
	
	sampRegisterChatCommand('settoken', function(text)
		if #text > 0 then
			addBankMessage(string.format('Вы изменили токен доступа к ВКонтакте'))
			cfg.nameRank[7] = text
		else
		addBankMessage('Используй /settoken [Токен]')
		end
	end)
	
	sampRegisterChatCommand('settag', function(text)
		if #text > 0 then
			addBankMessage(string.format('Вы изменили свой тег: {M}%s{W} -> {M}%s{W} ', cfg.nameRank[9], text))
			cfg.nameRank[9] = text
		else
		addBankMessage('Используй /settag [Админ Тег]')
		end
	end)
	...
	sampRegisterChatCommand('finv', function(id)
		...
		addBankMessage('Используй: /finv [id]')
		return
	end)
	...
	sampRegisterChatCommand('cj', function(id)
		...
		addBankMessage('Используй: /cj [id]')
		return
	end)
	...
	sampRegisterChatCommand('lwarn', function(args)
		...
		if id ~= nil and reason ~= nil then
			id = tonumber(id)
			if cfg.nameRank[7] ~= 'token_id' then
				local ttext = string.format('/warn %s +1 %s', formNick(id), reason)
				sampAddChatMessage(tag..'Вы выдали предупреждения '..rpNick(id)..' по причине: '..reason)
				requests.get("https://api.vk.com/method/messages.send?v=5.103&access_token="..cfg.nameRank[7].."&chat_id="..cfg.nameRank[8].."&message="..urlencode(u8(ttext)).."&random_id="..rnd)
				return
			end
			addBankMessage('Вы не ввели токен ВКонтакте! Используйте /settoken')
			return
		end
		addBankMessage('Используй: /lwarn [id] [причина]')
	end)
	...
	sampRegisterChatCommand('lunwarn', function(args)
		...
		if id ~= nil and reason ~= nil then
			...
			if cfg.nameRank[7] ~= 'token_id' then
				local ttext = string.format('/warn %s -1 %s', formNick(id), reason)
				sampAddChatMessage(tag..'Вы сняли предупреждения '..rpNick(id)..' по причине: '..reason)
				requests.get("https://api.vk.com/method/messages.send?v=5.103&access_token="..cfg.nameRank[7].."&chat_id="..cfg.nameRank[8].."&message="..urlencode(u8(ttext)).."&random_id="..rnd)
				return
			end
			addBankMessage('Вы не ввели токен ВКонтакте! Используйте /settoken')
		...
		addBankMessage('Используй: /lunwarn [id] [причина]')
	end)
	...
	sampRegisterChatCommand('vig', function(args)
		...
		if id ~= nil and reason ~= nil then
			...
			if cfg.nameRank[7] ~= 'token_id' then
				...
				sampAddChatMessage(tag..'Вы выдали выговор '..rpNick(id)..' по причине: '..reason)
				requests.get("https://api.vk.com/method/messages.send?v=5.103&access_token="..cfg.nameRank[7].."&chat_id="..cfg.nameRank[8].."&message="..urlencode(u8(ttext)).."&random_id="..rnd)
				return
			end
			addBankMessage('Вы не ввели токен ВКонтакте! Используйте /settoken')
		...
		addBankMessage('Используй: /vig [id] [причина]')
	end)
	...
	sampRegisterChatCommand('unvig', function(args)
		...
		if id ~= nil and reason ~= nil then
			...
			if cfg.nameRank[7] ~= 'token_id' then
				local ttext = string.format('/vig %s -1 %s', formNick(id), reason)
				sampAddChatMessage(tag..'Вы сняли выговор '..rpNick(id)..' по причине: '..reason)
				requests.get("https://api.vk.com/method/messages.send?v=5.103&access_token="..cfg.nameRank[7].."&chat_id="..cfg.nameRank[8].."&message="..urlencode(u8(ttext)).."&random_id="..rnd)
				return
			end
			addBankMessage('Вы не ввели токен ВКонтакте! Используйте /settoken')
		...
		addBankMessage('Используй: /unvig [id] [причина]')
	end)
	...
	sampRegisterChatCommand('bug', function(id)
		...
		addBankMessage('Используй: /bug [id]')
		return
	end)
	...
	sampRegisterChatCommand('form', function(text)
		if cfg.nameRank[7] ~= 'token_id' then
			local ttext = string.format('!form %s // '..cfg.nameRank[9], text)
	...
	sampRegisterChatCommand('sellcar', function(id)
		addBankMessage('Не верно! Используйте /sellcarto(если хотите слить в госс то /slitcar)')
	end)
	...
	sampRegisterChatCommand('mb', function(id)
		play_message(MsgDelay.v, true, {{ "/members" } })
	end)
	
	sampRegisterChatCommand('normgovVIDEO', function(id)
		sampAddChatMessage('{2090FF}[D] Губернатор Alex_Benson[666]: [Пра-во] - [ВСЕМ] Занимаю гос.волну, просьба не перебивать')
		sampAddChatMessage('{0c5ba7}Гос.Новости Alex_Benson[666]: [Пра-во] Уважаемые граждане, в данный момент')
		sampAddChatMessage('{0c5ba7}Гос.Новости Alex_Benson[666]: [Пра-во] Проходит Собеседование в правительства штата Гилберт.')
		sampAddChatMessage('{0c5ba7}Гос.Новости Alex_Benson[666]: [Пра-во] При себе иметь: паспорт, лицензии и мед.карту!')
		sampAddChatMessage('{2090FF}[D] Губернатор Alex_Benson[666]: [Пра-во] - [ВСЕМ] Освободил гос.волну вещания.')
	end)
	
	sampRegisterChatCommand('cmdgovVIDEO', function(id)
		addBankMessage('/d [Пра-во] - [ВСЕМ] Занимаю гос.волну, просьба не перебивать')
		sampAddChatMessage('{2090FF}[D] Губернатор Alex_Benson[666]: [Пра-во] - [ВСЕМ] Занимаю гос.волну, просьба не перебивать')
		addBankMessage('/gov [Пра-во] Уважаемые граждане, в данный момент')
		sampAddChatMessage('{0c5ba7}Гос.Новости Alex_Benson[666]: [Пра-во] Уважаемые граждане, в данный момент')
		addBankMessage('/gov [Пра-во] Проходит Собеседование в правительства штата Гилберт.')
		sampAddChatMessage('{0c5ba7}Гос.Новости Alex_Benson[666]: [Пра-во] Проходит Собеседование в правительства штата Гилберт.')
		addBankMessage('/gov [Пра-во] При себе иметь: паспорт, лицензии и мед.карту!')
		sampAddChatMessage('{0c5ba7}Гос.Новости Alex_Benson[666]: [Пра-во] При себе иметь: паспорт, лицензии и мед.карту!')
		addBankMessage('/d [Пра-во] - [ВСЕМ] Освободил гос.волну вещания.')
		sampAddChatMessage('{2090FF}[D] Губернатор Alex_Benson[666]: [Пра-во] - [ВСЕМ] Освободил гос.волну вещания.')
	end)
	
	sampRegisterChatCommand('errorgovVIDEO', function(id)
		sampAddChatMessage('{2090FF}[D] Губернатор Alex_Benson[666]: [Пра-во] - [ВСЕМ] Занимаю гос.волну, просьба не перебивать')
		sampAddChatMessage('{0c5ba7}Гос.Новости Alex_Benson[666]: [Пра-во] Уважаемые граждане, в данный момент')
		sampAddChatMessage('{FF5555}[Ошибка] {FFFFFF}Не флуди!')
		sampAddChatMessage('{0c5ba7}Гос.Новости Alex_Benson[666]: [Пра-во] При себе иметь: паспорт, лицензии и мед.карту!')
		sampAddChatMessage('{2090FF}[D] Губернатор Alex_Benson[666]: [Пра-во] - [ВСЕМ] Освободил гос.волну вещания.')
	end)
	
	sampRegisterChatCommand('kdgovVIDEO', function(id)
		sampAddChatMessage('{2090FF}[D] Губернатор Alex_Benson[666]: [Пра-во] - [ВСЕМ] Занимаю гос.волну, просьба не перебивать')
		sampAddChatMessage('{0c5ba7}Гос.Новости Alex_Benson[666]: [Пра-во] Уважаемые граждане, в данный момент')
		sampAddChatMessage('{FF5555}[Ошибка] {FFFFFF}Все организации имеют право только на 3 гос. строки в 10 минут!')
		sampAddChatMessage('{FF5555}[Ошибка] {FFFFFF}Все организации имеют право только на 3 гос. строки в 10 минут!')
		sampAddChatMessage('{2090FF}[D] Губернатор Alex_Benson[666]: [Пра-во] - [ВСЕМ] Освободил гос.волну вещания.')
	end)
	
	sampRegisterChatCommand('clearVIDEO', function(id)
		sampAddChatMessage('{2090FF}')
		sampAddChatMessage('{2090FF}')
		sampAddChatMessage('{2090FF}')
		sampAddChatMessage('{2090FF}')
		sampAddChatMessage('{2090FF}')
	...
	sampRegisterChatCommand('jobp', function(id)
		play_message(MsgDelay.v, true, {{ "/jobprogress" } })
	end)
	...
	while true do
		...
		if testCheat('LH') and not bank.v and not int_bank.v then
			type_window.v = 1
			bank.v = true
		end
		...
		if not bratishka and cfg.main.colorBenson ~= 4288216832 then 
			cfg.main.colorBenson = 4288216832 
		end
		...
		if bank.v or int_bank.v or infoupdate.v then 
...
function se.onServerMessage(clr, msg) -- хук чата
	...
	if msg:find('^%[R%]') and clr == 0x2DB043FF then
		if msg:find('%[Объявление%]') and {msg:find(cfg.nameRank[9], 1, true) or msg:find(cfg.nameRank[10], 1, true) or bratishka} then
			...
			local nR, nG, nB, nA = imgui.ImColor(cfg.main.colorBenson):GetRGBA()
			local nick = msg:match('([A-z0-9_]+%[%d+%]):')
			if nick and nick:find('_Benson', 1, true) then
		...
		for line in text:gmatch('[^\r\n]+') do
			...
			if rank then
				...
				if rank ~= cfg.main.rank then
					cfg.main.rank = rank
					if inicfg.save(cfg, 'Esida_Config.ini') and await['rank_update'] then 
...
function imgui.OnDrawFrame()
	...
	if int_bank.v then
		...
		imgui.Begin(u8'##IntMenuBank', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoScrollbar)
			...
			if TypeAction.v == 2 or TypeAction.v == 3 then
				...
				if TypeAction.v == 1 then
					if not go_credit.v then
						...
						if cfg.main.rank > 1 then
							if imgui.Button(u8('Вылечить пациента ')..fa.ICON_FA_MINUS_CIRCLE, imgui.ImVec2(250, 30)) then
								await['dep_minus'] = os.clock()
								addBankMessage('Используйте /hl [ID]!')
							end
						else
							imgui.DisableButton(u8('Вылечить пациента ')..fa.ICON_FA_LOCK, imgui.ImVec2(250, 30))
							imgui.Hint('depositdown5rank', u8'Доступно с 2+ ранга')
						end
						imgui.SetCursorPos(imgui.ImVec2(165, 80))
						if cfg.main.rank > 2 then
							if imgui.Button(u8('Выдать мед карту ')..fa.ICON_FA_PLUS_CIRCLE, imgui.ImVec2(250, 30)) then
								await['dep_plus'] = os.clock()
								addBankMessage('Используйте /mc [ID]!')
							end
						else
							imgui.DisableButton(u8('Выдать мед карту ')..fa.ICON_FA_LOCK, imgui.ImVec2(250, 30))
							imgui.Hint('depositeup5rank', u8'Доступно с 3+ ранга')
						end
						imgui.SetCursorPos(imgui.ImVec2(165, 115))
						if cfg.main.rank > 3 then
							if imgui.Button(u8('Вылечить от нарко ')..fa.ICON_FA_CALCULATOR, imgui.ImVec2(-1, 30)) then
								addBankMessage('Используйте /narko [ID]!')
							end
						else
							imgui.DisableButton(u8('Вылечить от нарко ')..fa.ICON_FA_LOCK, imgui.ImVec2(-1, 30))
							imgui.Hint('credit6rank', u8'Доступно с 4+ ранга')
						end
						imgui.SetCursorPos(imgui.ImVec2(165, 150))
						if cfg.main.rank > 4 then
							if imgui.Button(u8('Первый укол вакцины ')..fa.ICON_FA_MONEY_CHECK, imgui.ImVec2(-1, 30)) then
								await['debt'] = os.clock()
								addBankMessage('Используйте /vac1 [ID]!')
							end
						else
							imgui.DisableButton(u8('Первый укол вакцины ')..fa.ICON_FA_LOCK, imgui.ImVec2(-1, 30))
							imgui.Hint('debt5rank', u8'Доступно с 5+ ранга')
						end
						imgui.SetCursorPos(imgui.ImVec2(165, 185))
						if cfg.main.rank > 4 then
							if imgui.Button(u8('Второй укол вакцины ')..fa.ICON_FA_PIGGY_BANK, imgui.ImVec2(-1, 30)) then
								await['get_money'] = os.clock()
								addBankMessage('Используйте /vac2 [ID]!')
							end
						else
							imgui.DisableButton(u8('Второй укол вакцины ')..fa.ICON_FA_LOCK, imgui.ImVec2(-1, 30))
							imgui.Hint('howmuchmoney5rank', u8'Доступно с 5+ ранга')
						end
						imgui.SetCursorPos(imgui.ImVec2(165, 220))
						if cfg.main.rank > 4 then
							if imgui.Button(u8('Вывести тату ')..fa.ICON_FA_CREDIT_CARD, imgui.ImVec2(-1, 30)) then
								await['card_create'] = os.clock()
								addBankMessage('Используйте /tatu [ID]!')
							end
						else
							imgui.DisableButton(u8('Вывести тату ')..fa.ICON_FA_LOCK, imgui.ImVec2(-1, 30))
						...
						if cfg.main.rank > 4 then
							if imgui.Button(fa.ICON_FA_CLOCK, imgui.ImVec2(35, 65)) then
								...
								play_message(MsgDelay.v, false, {
									{ "/me {sex:посмотрел|посмотрела} на часы" },
									{ "/time"}
								})
							end
						else
			...
			if TypeAction.v == 2 or TypeAction.v == 3 then
				...
				imgui.RadioButton(u8("Помощь по командам"), TypeAction, 1)
	...
	if bank.v then
		...
		imgui.BeginGroup()
			...
			imgui.TextColored(imgui.GetStyle().Colors[imgui.Col.ButtonHovered], fa.ICON_FA_UNIVERSITY.. 'Esida')
			local len_title = imgui.CalcTextSize(fa.ICON_FA_UNIVERSITY.. 'Esida').x
			imgui.PopFont()

			imgui.PushFont(font[11])
			imgui.SetCursorPos( imgui.ImVec2(len_title + 15, 27) )
			imgui.TextColored(imgui.ImVec4(0.5, 0.5, 0.5, 1.0), 'v'..thisScript().version .. (devmode and ' (Insider Preview)' or ''))
			imgui.PopFont()
		...
		imgui.BeginGroup()
			...
			local str_name = string.gsub(sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))), "_", " ")
				...
				if sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))) == 'Alex_Benson' then
					imgui.CenterTextColoredRGB(sc ..'{FF5555}Developer')
				end
				imgui.CenterTextColoredRGB(sc ..str_name)
				imgui.PopFont()
				imgui.PushFont(font[11])
				imgui.CenterText(u8(string.format(cfg.nameRank[10])))
				imgui.CenterText(u8(string.format(cfg.nameRank[9])))
				imgui.PopFont()
				imgui.Hint('updateprogress', u8'Что бы изменить должность используйте /setpost')
			imgui.EndChild()
			...
			if imgui.Button(u8('О скрипте ')..fa.ICON_FA_INFO_CIRCLE, imgui.ImVec2(180, 30)) then
				type_window.v = 3
			end
			if imgui.Button(u8('Благодарности ')..fa.ICON_FA_USERS, imgui.ImVec2(180, 30)) then
				type_window.v = 7
			end
		imgui.EndGroup()
		imgui.SameLine()
		...
		if type_window.v == 5 then -- Бинды
			if #cfg.Binds_Name > 0 then
				...
				imgui.PopFont()
				imgui.NewLine()
				imgui.CenterTextColoredRGB('У вас нет биндов!')
				imgui.NewLine()
		...
		if type_window.v == 3 then -- О скрипте
			imgui.CenterTextColoredRGB(mc..'О скрипте')
				imgui.CenterTextColoredRGB('Автор скрипта: ' .. mc..'Alex_Benson')
				...
				if imgui.Button(fa.ICON_FA_LINK..u8(" ВКонтакте"), imgui.ImVec2(90, 25)) then
					...
					setClipboardText("https://vk.com/nik446")
				end
				...
				if imgui.Button(fa.ICON_FA_LINK..u8(" Discord"), imgui.ImVec2(90, 25)) then
					addBankMessage('Ссылка скопирована в буфер обмена')
					setClipboardText("Alex_Benson#7293")
				end
				...
				if imgui.Button(fa.ICON_FA_LINK..u8(" Telegram"), imgui.ImVec2(90, 25)) then
					addBankMessage('Ссылка скопирована в буфер обмена')
					setClipboardText("https://t.me/hit_vandal")
				end
				imgui.PopStyleColor(3)
				imgui.PopStyleVar()

				imgui.SetCursorPosX((imgui.GetWindowWidth() - 150) / 2)
				if imgui.RedButton(u8'Удалить Esida', imgui.ImVec2(150, 20)) then
				...
				if imgui.BeginPopupModal('##deleteBankHelper', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoTitleBar) then
					...
					imgui.CenterText(u8'Удаление Esida', imgui.ImVec4(0.8, 0.3, 0.3, 1.0))
					imgui.PopFont()
					imgui.Spacing()
					imgui.CenterText(u8'Вы действительно хотите удалить Esida?')
					...
					if imgui.RedButton(u8'Удалить', imgui.ImVec2(150, 30)) then
						...
						os.remove(getWorkingDirectory() .. '\\config\\Esida_Config.ini')
						unload(false)
					end
					imgui.SameLine()
			...
			imgui.SameLine()
			if imgui.GreenButton(u8('Список изменений')..fa.ICON_FA_DOWNLOAD, imgui.ImVec2(imgui.GetWindowWidth() / 2 - 12, 30)) then
				 infoupdate.v = true
			end
			post()
		end
		if type_window.v == 7 then -- Благодарности
			imgui.CenterTextColoredRGB(mc..'Благодарности')
				imgui.CenterTextColoredRGB('За основу для скрипта: ' .. mc..'COSMO')
				imgui.CenterTextColoredRGB('Помог в изучение lua кодинга: ' .. mc..'Виктор Сорокин')
				imgui.CenterTextColoredRGB('Помог с интеграции ВКонтакте: ' .. mc..'Дмитрий Кубров')
				imgui.CenterTextColoredRGB('Помог исправить критические баги: ' .. mc..'Christopher_Dills')
				imgui.CenterTextColoredRGB('Помог исправить мелкие ошибки: ' .. mc..'Владимир Малевский')
		end
		...
		if type_window.v == 1 then -- Настройки
			...
			imgui.NewLine()
				...
				if imgui.Checkbox(u8'Авто-проверка обновлений', loginupdate) then
					...
					if inicfg.save(cfg, 'Esida_Config.ini') then
				...
				if devmode then
					imgui.TextColoredRGB(mc..'Цвета чатов')
				...
				if bratishka then 
					if imgui.ColorEdit4(u8'Цвет ника в чате орг', colorBenson, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoAlpha) then
						local clr = imgui.ImColor.FromFloat4(colorBenson.v[1], colorBenson.v[2], colorBenson.v[3], colorBenson.v[4]):GetU32()
						cfg.main.colorBenson = clr
						inicfg.save(cfg, 'Esida_Config.ini')
					end
					imgui.SameLine()
					imgui.TextDisabled('(?)')
					...
					imgui.SameLine(imgui.GetWindowWidth() - 150)
					if imgui.Button(u8("Тест##BensonCol"), imgui.ImVec2(50, 20)) then
						local cr, cg, cb, ca = imgui.ImColor(cfg.main.colorBenson):GetRGBA()
					...
					imgui.SameLine(imgui.GetWindowWidth() - 95)
					if imgui.Button(u8("Стандартный##BensonCol"), imgui.ImVec2(90, 20)) then
						cfg.main.colorBenson = 4294940979
						if inicfg.save(cfg, 'Esida_Config.ini') then 
							addBankMessage('Стандартный цвет ника в чате орг. восстановлен!')
							colorBenson = imgui.ImFloat4(imgui.ImColor(cfg.main.colorBenson):GetFloat4())
						end
					end
				else
					imgui.ColorEdit4(u8'##Цвет ника Disabled', colorBenson, imgui.ColorEditFlags.NoInputs + imgui.ColorEditFlags.NoPicker + imgui.ColorEditFlags.NoTooltip)
					...
					imgui.Hint('colormynick', u8'Функция доступна пользователям с фамилией Benson\nНапример: Bob_Benson')
					imgui.SameLine(imgui.GetWindowWidth() - 150)
					imgui.DisableButton(u8"Тест##BensonColDisabled", imgui.ImVec2(50, 20))
					imgui.SameLine(imgui.GetWindowWidth() - 95)
					imgui.DisableButton(u8"Стандартный##BensonColDisabled", imgui.ImVec2(90, 20))
				end
			if devmode then
					imgui.TextColoredRGB('{868686}(Подробнее)')
					imgui.Hint('setform', u8'Что бы не писать вручную длинную форму\nдостаточно в строку чата написать например !лекция\nи скрипт сам за вас вставит в строку чата форму доклада')
					imgui.PushItemWidth(280)
		...
		if type_window.v == 6 then 
			...
			imgui.Hint('aboutrepository', u8'Все скрипты проверены автором Esida!')
			imgui.Separator()
			imgui.Repository('WTD Changer', 'Esida WTD Changer.lua', 'Скрипт, который позволяет менять время и погоду.\nСохраняется после перезахода\nСервер не сможет сменить вам погоду.', '/shelp', 'https://drive.google.com/u/0/uc?id=1IdulQclYQurLzzQbJD7RrA6kOwlOdAYu&export=download')
			end
		imgui.EndChild()
		imgui.EndGroup()
		imgui.End()
			...
			if v.active then
				...
				imgui.Begin(u8'##notifycard'..k, _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar)
					...
					imgui.BeginGroup()
						...
						imgui.PushFont(font[15])
						imgui.TextColoredRGB('{006AC2}Esida')
						imgui.PopFont()
...
function go_expel(playerId)
	...
	if target_color ~= self_color then
		if not sampIsPlayerPaused(playerId) then
			play_message(MsgDelay.v, true, {
				{ "Мне придётся вывести вас из здания больницы." },
				{ "/me {sex:схватил|схватила} за руку %s и {sex:повёл|повела} к выходу", rpNick(playerId) },
				{ "/todo А теперь подумайте над своим поведением!*закрывая дверь больницы.." },
		...
		addBankMessage('Похоже, вы пытаетесь выгнать сотрудника больницы...')
	end
end
...
function Window_Info_Update()
	if infoupdate.v then 
		...
		local col_line_tree = 0xFF666666
		DL:AddLine(imgui.ImVec2(p.x + offset, p.y), imgui.ImVec2(p.x + offset, p.y + imgui.GetScrollMaxY() + imgui.GetWindowHeight()), col_line_tree, 10);
		...
		imgui.BeginGroup()
			for i = #changelog, 1, -1 do
				...
				imgui.BeginGroup()
					...
					local ccol
					if cl.date ~= nil then
						ccol = i == #changelog and 0xFFFF8000 or 0xFF222222
					else
						ccol = i == #changelog and 0xFF0090CC or 0xFF222222
					end
					local csiz = i == #changelog and 12 or 10
					local col_circle_out = 0xFF444444
					DL:AddCircleFilled(imgui.ImVec2(p.x + offset + 1, p2.y + 10), csiz, col_circle_out)
					DL:AddCircleFilled(imgui.ImVec2(p.x + offset + 1, p2.y + 10), csiz - 3, ccol)
					if cl.date ~= nil then
						...
						imgui.TextColoredRGB('{CC9000}Версия ' .. cl.version .. '-Beta')
					end
					imgui.PopFont()
					if cl.comment ~= "" then
...
function helpCommands()
	if imgui.BeginPopupModal(u8("Все команды скрипта"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar) then
		imgui.PushFont(font[20])
		imgui.CenterTextColoredRGB(mc..'Все доступные команды')
		imgui.PopFont()
		imgui.Spacing()
		imgui.TextColoredRGB(sc..'Команды для администрации:')
		imgui.SetCursorPosX(20)
		imgui.BeginGroup()
			imgui.TextColoredRGB(mc..'/lid{SSSSSS} - Основное меню скрипта (либо "LH" как чит-код)')
		...
		imgui.TextColoredRGB(sc..'Команды для работы с ВКонтакте:')
			imgui.TextColoredRGB(mc..'/form [Форма]{SSSSSS} - Отправить форму')
		...
		imgui.TextColoredRGB(sc..'Команды для взаимодействие с игроками:')
		...
		imgui.TextColoredRGB(sc..'Команды для отказа игрокам:')
		...
		imgui.EndGroup()
		imgui.Spacing()
		imgui.TextColoredRGB(sc..'Команды для следящих:')
		imgui.SetCursorPosX(20)
		imgui.BeginGroup()
		...
		imgui.TextColoredRGB(sc..'Команды для взаимодействие с фамой:')
			imgui.TextColoredRGB(mc..'/finv [ID]{SSSSSS} - Принять человека в семью')
		if devmode then
		imgui.TextColoredRGB(sc..'Команды разработчика:')
...
function checkData()
	local dir = getWorkingDirectory() .. '\\Esida'
	if not doesDirectoryExist(dir) then
		if createDirectory(dir) then
			log(string.format('Создана директория %s', dir))
		end
	end

	log('Проверка наличия лекций..')
	if not doesFileExist(lect_path) then
		lections = lections_default
		local file = io.open(lect_path, "w")
		file:write(encodeJson(lections))
		file:close()
		log('Файл лекций не найден, создан файл')
	else
	...
	for name, text in pairs(files) do 
		if not doesFileExist(dir .. '\\' .. name) then
			log(string.format('Создан файл «%s»', name))
...
changelog = {
	[1] = {
		version = '2.0',
		comment = 'Начало...',
		date = os.time({day = '21', month = '01', year = '2022'}),
		log = {
	...
	[2] = {
		version = '2.1',
		comment = 'bug fix',
		date = os.time({day = '22', month = '01', year = '2022'}),
		log = {
	...
	[4] = {
		version = '2.2',
		comment = 'bug fix',
		date = os.time({day = '23', month = '01', year = '2022'}),
		log = {
			'Удалена команда /kip',
		},
		patches = {
...
charter_default = [[ Скопируйте суда сами вдруг будет считаться сливом админ инфы ]]
...
 
{0088C2}Вирусолог [8] -> Зам.Глав.Врача [9]
	...
	data = {
		{
			...
			text = {
				"Сейчас Вы прослушаете лекцию о том, как должен выглядеть любой сотрудник больницы в рабочее время",
				"Каждый сотрудник, приходя на смену должен переодеться в рабочую форму, взять медикаменты и рацию",
		...
		},
		{
			...
			text = {
				...
				"Если вы хотите отлучится по личных делам, например покушать или уехать в банк",
		...
		{
			name = "Правила нашей больницы",
			text = {
				"Сегодняшняя лекция посвящена некоторым правилам нашего больницы",
				"Итак, нельзя лечить пациентов где попала, для этого ведь есть палаты",
				"Покидать банк и тем более брать транспорт без разрешения запрещается",
				"За это наше руководство будет строго наказывать",
				"Полный список правил больницы выложен на официальном портале нашего штата",
...
function autoupdate(json_url)
	...
	log('Начало проверки обновления')
				...
				if f then
					...
					if updateversion ~= thisScript().version then
						lua_thread.create(function()
							local dlstatus = require('moonloader').download_status
							local color = -1
							log('Найдено обновление: '..thisScript().version..' -> '..updateversion..'! Загрузка..')
							wait(250)
							downloadUrlToFile(updatelink, thisScript().path,
								function(id3, status1, p13, p23)
									if status1 == dlstatus.STATUS_DOWNLOADINGDATA then
									elseif status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
										log('Загрузка окончена. Скрипт обновлен на версию '..mc..updateversion)
										goupdatestatus = true
									...
									if status1 == dlstatus.STATUSEX_ENDDOWNLOAD then
										if goupdatestatus == nil then
											log('Скрипт не смог обновится на версию '..updateversion)
											update = false
										end
									end
								end
						...
						update = false
						log('Версии совпадают. Обновлений нет')
						addBankMessage('Обновлений не найдено')
					end
				end
			else
				log('Не удалось получить JSON таблицу', 0xFF0000)
